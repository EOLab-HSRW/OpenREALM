...
TEST(CvGridMap, LayerManagement)
{
  ...
  map.add("a", cv::Mat(map.size(), CV_8UC1, 50), cv::INTER_AREA);
  map.add(CvGridMap::Layer{"b", cv::Mat(map.size(), CV_8UC1, 100), cv::INTER_AREA});
  ...
  EXPECT_EQ(map.getLayer("b").interpolation, cv::INTER_AREA);

  std::vector<std::string> layer_names = map.getAllLayerNames();
  EXPECT_EQ(layer_names[0], "b");

  // Overwrite of existing data
  map.add(CvGridMap::Layer{"b", cv::Mat(map.size(), CV_8UC1, 125), cv::INTER_AREA});
  ...
  map.add(CvGridMap::Layer{"b", cv::Mat(), cv::INTER_AREA}, true);
...
TEST(CvGridMap, Clone)
{
  CvGridMap map(cv::Rect2d(0, 0, 20, 30), 1.0);
  map.add("a", cv::Mat(map.size(), CV_8UC1, 50), cv::INTER_AREA);
  map.add("b", cv::Mat(map.size(), CV_8UC1, 50), cv::INTER_AREA);
...
TEST(CvGridMap, ExtractSubmap)
{
  ...
  CvGridMap map(cv::Rect2d(0, 0, 20, 30), 1.0);
  map.add("a", cv::Mat(map.size(), CV_8UC1, 50), cv::INTER_AREA);
  map.add("b", cv::Mat(map.size(), CV_8UC1, 100), cv::INTER_AREA);
